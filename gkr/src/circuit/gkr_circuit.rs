...
use crate::poly::multivariate::MultivariatePolynomial;
use crate::poly::univariate::CoefficientBasis;
...
impl<F: Field> Wire<F> {
    ...
    fn max_gate_degree(c: &Circuit<F>) -> usize {
        let mut res = 1;
        for i in c.iter() {
            if !i.is_input() {
...
type WireAssignment<F> = BTreeMap<Wire<F>, MultivariatePolynomial<F, CoefficientBasis>>;
